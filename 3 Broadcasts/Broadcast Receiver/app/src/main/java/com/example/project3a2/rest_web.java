...
public class rest_web extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        array_len = RestActivity.rest_web_arr.length;
        ...
        model.getSelectedItem().observe(getViewLifecycleOwner(), item->{
            if(item < 0 || item >= array_len)
                return;
            curr_index = item;
            open.loadUrl(RestActivity.rest_web_arr[curr_index]);

        });
        open = getActivity().findViewById(R.id.web_1);
    }
}